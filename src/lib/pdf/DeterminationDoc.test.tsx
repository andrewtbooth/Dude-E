...
describe("Chapter 98 on the document", () => {
  /**
   * The determination states what the product *is*, which is what a product
   * library carries. A Chapter 98 provision states something about one
   * importation of it. A reader months from now sees a page headed
   * DETERMINATION with a code on it; a Chapter 98 number printed anywhere near
   * that without saying which is which reads as part of the answer.
   */
  const withProvision = () => {
    const view = sampleDeterminationView();
    return {
      ...
      chapter98Provisions: [
        {
          hts_code: "9813.00.20",
          provision: "Temporary importation under bond — samples for taking orders",
          applies_when:
            "The samples are imported solely to solicit orders, are not sold, and are exported or destroyed within the bond period.",
        },
      ],
    };
  };

  it("prints it under its own heading, apart from the determination", async () => {
    const text = squashed(await textOf(withProvision()));
    expect(text).toContain(squashed("CLAIMABLE ON A PARTICULAR ENTRY"));
    expect(text).toContain(squashed("NOT PART OF THIS CLASSIFICATION"));
    expect(text).toContain("9813.00.20");
  });

  it("states the conditions rather than implying the provision applies", async () => {
    const text = await textOf(withProvision());
    expect(text).toContain("Applies when:");
    expect(text).toContain("exported or destroyed within the bond period");
  });

  it("stays silent when nothing was named", async () => {
    // The normal case. A section on every determination is furniture, and this
    // one exists to draw a distinction that only matters when it is real.
    const text = squashed(await textOf(sampleDeterminationView()));
    expect(text).not.toContain(squashed("CLAIMABLE ON A PARTICULAR ENTRY"));
  }, 30_000);
});
