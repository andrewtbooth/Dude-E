...
import {
  ...
  splitSectionNotes,
  tableHeaderIndex,
} from "../src/lib/hts/notes";
import { parseChapter99Coverage } from "../src/lib/hts/chapter99";
...
import type { Chapter99Coverage } from "../src/lib/hts/chapter99";
import type {
  HtsLine,
  HtsNote,
  HtsusManifest,
...
/**
 * Chapter 99 coverage, read from the subchapter U.S. Notes.
 *
 * Uses the *full* Chapter 99 document rather than the trimmed notes: the
 * subchapter notes that enumerate Section 301 and 232 coverage sit after the
 * first tariff table, which is exactly why the ordinary notes path never saw
 * them and the app reported "no Chapter 99 provisions" for staple covered
 * goods.
 */
async function fetchChapter99Coverage(): Promise<Chapter99Coverage[]> {
  const url = `${BASE_URL}/file?release=currentRelease&filename=${encodeURIComponent("Chapter 99")}`;
  try {
    const response = await fetchWithRetry(url, 2);
    const bytes = Buffer.from(await response.arrayBuffer());
    if (!isPdf(bytes)) {
      warn("Chapter 99 document was not a PDF; coverage screening unavailable.");
      return [];
    }
    const doc = await getDocumentProxy(new Uint8Array(bytes), {
      verbosity: PDFJS_ERRORS_ONLY,
    ...
    const { text } = await extractText(doc, { mergePages: true });
    const rows = parseChapter99Coverage(text);
    if (rows.length === 0) {
      warn(
        "Chapter 99 notes parsed to zero coverage entries. Section 301 and 232 " +
          "screening will be footnote-only, which misses most covered goods.",
      );
    }
    return rows;
  } catch (error) {
    warn(
      `Chapter 99 coverage notes unavailable (${error instanceof Error ? error.message : String(error)}). ` +
        `Trade-action screening falls back to published footnotes alone.`,
    );
    return [];
  }
}
...
async function main(): Promise<void> {
  ...
  if (chapter99Coverage.length > 0) {
    const subheadings = new Set(chapter99Coverage.map((r) => r.baseDigits)).size;
    log(`  ${subheadings} subheadings enumerated across Chapter 99 notes`);
  }
  ...
  log(
    `  ch99 cover: ${new Set(chapter99Coverage.map((r) => r.baseDigits)).size} subheadings`,
  );
  log(
